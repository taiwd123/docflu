...
async function syncReverse(dryRun = false) {
  ...
  try {
    ...
    const htmlToMarkdownParser = new HtmlToMarkdownParser(projectRoot, confluenceClient);
    ...
    for (const [filePath, pageInfo] of allPages) {
      ...
      try {
        // Get page from Confluence with full content AND attachments
        const confluencePage = await confluenceClient.getPageWithAttachments(confluenceId);
        ...
        console.log(chalk.blue(`🔄 Processing updated page: ${pageInfo.title} (v${lastSyncVersion} → v${currentVersion})`));
        
        // Page was updated in Confluence, convert it back to Markdown
        const confluenceContent = confluencePage.body?.storage?.value || '';
        const attachments = confluencePage.attachments || [];
        const absoluteFilePath = path.resolve(projectRoot, filePath);
        
        console.log(chalk.blue(`📎 Found ${attachments.length} attachments for page: ${pageInfo.title}`));
        ...
        // Convert Confluence content to Markdown with attachment processing
